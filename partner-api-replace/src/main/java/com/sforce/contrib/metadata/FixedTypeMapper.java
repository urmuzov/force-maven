...
public class FixedTypeMapper extends TypeMapper {
    ...
    Object readSingle(XmlInputStream in, TypeInfo typeInfo, Class type)
        ...
        try {
            XMLizable result;
            if (type == Picklist.class) {
                result = new FixedPicklist();
            } else {
                result = (XMLizable) type.newInstance();
            }
    ...
    private String readEnum(XmlInputStream in, TypeInfo typeInfo, Class<?> type) throws IOException, ConnectionException {
        ...
        // Наш хот фикс -->
        if (type == Encoding.class) {
            if (s.equals("UTF-8")) {
                return Encoding.UTF_8.name();
            } else if (s.equals("ISO-8859-1")) {
                return Encoding.ISO_8859_1.name();
            } else if (s.equals("ISO-2022-JP")) {
                return Encoding.ISO_2022_JP.name();
            } else if (s.equals("EUC-JP")) {
                return Encoding.EUC_JP.name();
            } else if (s.equals("BIG5-HKSCS")) {
                return Encoding.Big5_HKSCS.name();
            } else if (s.equals("x-SJIS_0213")) {
                return Encoding.x_SJIS_0213.name();
            }
        }
        // <--
